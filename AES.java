...
public class AES {
	...
	public static void main(String[] args) {
		...
		roundKey = setUpArray (key, 15*4);
		int[][] cipher = setUpArray (key, 8);
		decrypt = args[0].equals("d"); 
		filename = (decrypt) ? filename + ".dec" : filename + ".enc";
		...
		try {
			...
			System.out.println(printMatrix(cryptMatrix));
			System.out.println("Original CipherKey: ");
			System.out.println(printMatrix(cipher));  
			...
			{	
				...
				rconIdx = 0;
				line = in.readLine();
				out.write(printMatrix(cryptMatrix));
			}
			out.flush();
			out.close();
			in.close(); 
	...
	private static int[][] setUpArray(File file, int size) {
		BufferedReader in;
	...
	private static int setUpArray(String line, int size) {
		int [][] making = new int[4][size]; 
		int col = 0;
		int row = 0;

		for (int i = 0; i<line.length()-1 && i <32; i=i +2)
		{
			if (row == 4)
			{
				row = 0;
